...
const AIChatPage = ({ isTransitioningOut = false }: AIChatPageProps) => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {showWelcome && messages.length <= 1 && (
        <div className={`absolute inset-0 flex items-center justify-center pointer-events-none ${isTransitioningOut ? 'animate-slideOutUp' : ''}`}>
          <h1 className={`text-5xl font-bold text-white ${isTransitioningOut ? '' : 'animate-welcome'}`}>
            欢迎进入AI聊天室
          </h1>
        </div>
      )}
      ...
      {/* 毛玻璃对话界面 */}
      {!showWelcome && (
        <div className={isTransitioningOut ? 'animate-slideOutUp' : ''}>
          <ChatBox
            messages={messages}
          />
        </div>
      ...
      {/* 底部输入框 */}
      <div className={`absolute bottom-0 left-1/2 transform w-[80%] ${isTransitioningOut ? 'animate-slideOutDown' : 'animate-slideUp'}`}>
